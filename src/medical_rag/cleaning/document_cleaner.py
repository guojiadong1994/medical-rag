...
from medical_rag.parsing.models import CleanedDocument, CleanedPage, ParsedDocument, TextBlock
...
class DocumentCleaner:
    ...
    def _render_page_text(self, blocks: list[TextBlock], page_width: float) -> str:
        """Render cleaned line blocks as readable paragraphs for manual QA.

        The structured blocks remain untouched for the next heading/table stage. This
        preview renderer only repairs visual line wraps so reviewers can quickly spot
        extraction errors.
        """
        if not blocks:
            return ""
        ...
        for current in blocks[1:]:
            ...
            line_height = max(8.0, min(prev.bbox[3] - prev.bbox[1], current.bbox[3] - current.bbox[1]))
            close_line = -0.4 * line_height <= vertical_step <= 1.5 * line_height
            new_semantic_block = self._looks_like_semantic_boundary(current.text, current.is_bold)
            paragraph_indent = current.bbox[0] - prev.bbox[0] > 12.0 and prev.text.rstrip().endswith(("。", "！", "？", ";", "；"))
    ...
    def _find_repeated_margin_patterns(self, document: ParsedDocument) -> set[str]:
        ...
        for page in document.pages:
            ...
            for block in page.blocks:
                ...
                in_margin = y1 <= page.height * self.top_band_ratio or y0 >= page.height * self.bottom_band_ratio
                if not in_margin:
                    continue
    ...
    def _is_margin_noise(
        ...
        y0, y1 = block.bbox[1], block.bbox[3]
        in_margin = y1 <= page_height * self.top_band_ratio or y0 >= page_height * self.bottom_band_ratio
        if not in_margin:
            return False
